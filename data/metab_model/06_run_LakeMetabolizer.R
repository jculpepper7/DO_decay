...
kalman = metab(ts.data, method='kalman', 
                   wtr.name=paste('wtr',DO_depth,sep="_"), do.obs.name='do.obs', irr.name='irr')
names(attributes(kalman))
kalman.params <- attributes(kalman)$params
kalman <- cbind(kalman, kalman.params[,c(3:6)])
...
save(outputs,file=paste("07",lake,year,"LakeMetabolizer_outputs.Rdata",sep="_"))
###############################################################################################################################################################
##Plots
###############################################################################################################################################################
##plot metabolism estimates from 3 methods
par(mfrow=c(3,1))
plot(bookkeep$doy,bookkeep$GPP,type='b',pch=21,col='darkgreen',ylim=c(-0.5,1.5),xlim=c(225,280))
...
plot(bookkeep$doy,bookkeep$R,type='b',pch=21,col='brown',ylim=c(-1.5,0.5),xlim=c(225,280))
...
plot(bookkeep$doy,bookkeep$NEP,type='b',pch=21,col='darkblue',ylim=c(-0.5,0.5),xlim=c(225,280))
...
plot(bookkeep$datetime,bookkeep$GPP,type='b',ylim=c(-0.25,1.5),xlim=date.lims)
...
plot(bookkeep$datetime,bookkeep$R,type='b',ylim=c(-1.5,0.25),xlim=date.lims)
...
plot(bookkeep$datetime,bookkeep$NEP,type='b',ylim=c(-0.5,0.5),xlim=date.lims)
