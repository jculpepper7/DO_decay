...
soapstone_3 <- read_csv(here('data/raw/soapstone/soapstone_light_pendant_2021.08.28_raw.csv'), skip = 1) %>%
  clean_names() %>% #clean names from HOBO software default
...
soapstone_4 <- read_csv(here('data/raw/soapstone/soapstone_light_pendant_2021.10.16_raw.csv'), skip = 1) %>%
  clean_names() %>% #clean names from HOBO software default
  select(2,3,4) %>% #Only columns 2 and 3 have data
  rename(date_time = 1, temp_c = 2, light_intensity_lux = 3) %>% #rename columns to more intuitive names from HOBO software defaults
  mutate(date_time = mdy_hms(date_time),
...
soapstone_5 <- read_delim(here('data/raw/soapstone/soapstone_miniDOT_2021.08.28_raw.txt'), delim = ',', skip = 7) %>%
...
soapstone_6 <- read_delim(here('data/raw/soapstone/soapstone_miniDOT_2021.10.14_raw.txt'), delim = ',', skip = 7) %>%
  clean_names() %>%#clean names from miniDOT software default
  slice(-c(1)) %>%
  select(1,2,3,5,6,7) %>%
  rename(
...
write_csv(soapstone_3_cleaned, here('data/processed/soapstone/soapstone_light_pendant_2021.08.28_cleaned.csv'))
write_csv(soapstone_4_cleaned, here('data/processed/soapstone/soapstone_light_pendant_2021.10.16_cleaned.csv'))
write_csv(soapstone_5_cleaned, here('data/processed/soapstone/soapstone_miniDOT_2021.08.28_cleaned.csv'))
write_csv(soapstone_6_cleaned, here('data/processed/soapstone/soapstone_miniDOT_2021.10.14_cleaned.csv'))
